...
export function validateConfig(config: unknown): asserts config is ChatConfig {
  ...
  // authenticate is optional - if provided, validate it
  if (cfg.authenticate !== undefined && typeof cfg.authenticate !== "function") {
    throw new ConfigValidationError("authenticate must be a function if provided");
  }
